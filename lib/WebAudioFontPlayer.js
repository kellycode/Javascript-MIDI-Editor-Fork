    ...
    1: [function (require, module, exports) {
        ...
        function WebAudioFontLoader(player) {
            ...
            this.loaded = function (variableName) {
                ...
                let preset = window[variableName];
                for (let i in preset.zones) {
            ...
            this.instrumentInfo = function (n, baseURL) {
                let key = this.instrumentKeys()[n];
                let p = 1 * key.substring(0, 3);
                return {
                    variable: '_tone_' + key,
                    url: baseURL + '/' + key + '.js',
            ...
            this.findInstrument = function (program) {
                ...
                /*
                Old code
                for (let i = 0; i < this.choosenInfos.length; i++) {
            ...
            this.drumInfo = function (n, baseURL) {
                let key = this.drumKeys()[n];
                let p = 1 * key.substring(0, 2);
                return {
                    variable: '_drum_' + key,
                    url: baseURL + '/128' + key + '.js',
                    pitch: p,
                    title: this.drumTitles()[p]
                };
            };
            this.findDrum = function (nn) {
                for (let i = 0; i < this.drumKeys().length; i++) {
        ...
        let WebAudioFontLoader = require('./loader');
        let WebAudioFontChannel = require('./channel');
        let WebAudioFontReverberator = require('./reverberator')
