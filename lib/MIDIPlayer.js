        ...
        for (let t in song.tracks) {
            ...
            for (let i in track.notes) {
                if (track.notes[i].when >= start && track.notes[i].when < end) {
                    ...
                    if (this.monitor) {
                        this.monitor(audioContext, input, window[instr], when, track.notes[i].pitch, duration, v, track.notes[i].slides);
                    }
                }
            }
        }
        for (let b in song.beats) {
            ...
            for (let i in beat.notes) {
                if (beat.notes[i].when >= start && beat.notes[i].when < end) {
                    ...
                    if (this.monitor) {
                        this.monitor(audioContext, input, window[instr], when, beat.n, duration, v);
                    }
                }
            }
        }
